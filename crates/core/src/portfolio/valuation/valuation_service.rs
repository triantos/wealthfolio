...
impl ValuationServiceTrait for ValuationService {
    ...
    fn get_latest_valuations(
        ...
        if has_total {
            // Get the latest portfolio valuation and convert for compatibility.
            let portfolio_history = self
                .valuation_repository
                .get_portfolio_history(None, None)?;
            if let Some(latest) = portfolio_history.last() {
